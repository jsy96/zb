// POST /api/asr  { audio: base64(WAV), filename?, config? }  →  { ok, text }
// 实时录音的服务器识别模式：前端每十几秒送一小段 WAV，这里转发给 glm-asr
"use strict";
const { resolveConfig, readJsonBody, sendJson, callAsr } = require("./_shared");
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const text = await callAsr(cfg, wav, body.filename);
